...
class SlicerApplicationLogHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if hasattr(ctk, _("ctkErrorLogLevel")):
        ...
        self.origin = _("Python")
        self.category = _("Python")
...
def initLogging(logger):
    ...
    slicer.pythonApplicationLogHandler.setFormatter(logging.Formatter(_("%(message)s")))
...
def getSlicerRCFileName():
    ...
    will be used (_("~/.slicerrc.py"))."""
    import os
    rcfile = os.path.join(slicer.app.slicerHome, _(".slicerrc.py"))
    if not os.path.exists(rcfile):
        if _("SLICERRC") in os.environ:
            rcfile = os.environ[_("SLICERRC")]
        else:
            rcfile = os.path.expanduser(_("~/.slicerrc.py"))
...
def loadSlicerRCFile():
    _(_("If it exists, execute slicer resource script"))
    import os
    rcfile = getSlicerRCFileName()
    if os.path.isfile(rcfile):
        print(_("Loading Slicer RC file [%s]") % (rcfile))
...
class _Internal:
    ...
    def __init__(self):
        ...
        setattr(slicer, _("app"), _qSlicerCoreApplicationInstance)
        ...
        if hasattr(moduleManager, _("factoryManager")):
            factoryManager = moduleManager.factoryManager()
            factoryManager.connect(_("modulesRegistered(QStringList)"), self.setSlicerModuleNames)
            moduleManager.connect(_("moduleLoaded(QString)"), self.setSlicerModules)
            moduleManager.connect(_("moduleAboutToBeUnloaded(QString)"), self.unsetSlicerModule)

        # Retrieve current instance of the scene and set 'slicer.mrmlScene'
        setattr(slicer, _("mrmlScene"), slicer.app.mrmlScene())
        ...
        def _singleShot(msec, receiverOrCallable, member=None):
            _(_("Calls either a python function or a slot after a given time interval."))
            ...
            if callable(receiverOrCallable):
                timer.connect(_("timeout()"), receiverOrCallable)
            else:
                timer.connect(_("timeout()"), receiverOrCallable, member)
            timer.start(msec)

        qt.QTimer.singleShot = staticmethod(_singleShot)

    def setSlicerModuleNames(self, moduleNames):
        _(_("Add module names as attributes of module slicer.moduleNames"))
        for name in moduleNames:
            setattr(slicer.moduleNames, name, name)
            # HACK For backward compatibility with ITKv3, map "dwiconvert" module name to _("dicomtonrrdconverter")
            if name == _("DWIConvert"):
                setattr(slicer.moduleNames, _("DicomToNrrdConverter"), name)

    def setSlicerModules(self, moduleName):
        _(_("Add modules as attributes of module slicer.modules"))
        moduleManager = slicer.app.moduleManager()
        setattr(slicer.modules, moduleName.lower(), moduleManager.module(moduleName))
        # HACK For backward compatibility with ITKv3, map _("dicomtonrrdconverter") module to "dwiconvert"
        if moduleName == _("DWIConvert"):
            setattr(slicer.modules, _("dicomtonrrdconverter"), moduleManager.module(moduleName))

    def unsetSlicerModule(self, moduleName):
        _("""Remove attribute from ``slicer.modules``
        """)
        if hasattr(slicer.modules, moduleName + _("Instance")):
            delattr(slicer.modules, moduleName + _("Instance"))
        if hasattr(slicer.modules, moduleName + _("Widget")):
            delattr(slicer.modules, moduleName + _("Widget"))
