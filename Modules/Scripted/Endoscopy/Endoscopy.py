...
class EndoscopyWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        cameraNodeSelector.nodeTypes = [_('vtkMRMLCameraNode')]
        ...
        outputPathNodeSelector.objectName = _('outputPathNodeSelector')
    ...
    def onCreatePathButtonClicked(self):
        ...
        print(_("Calculating Path..."))
        result = EndoscopyComputePath(fiducialsNode)
        print(_("-> Computed path contains %d elements") % len(result.path))

        print(_("Create Model..."))
        model = EndoscopyPathModel(result.path, fiducialsNode, outputPathNode)
        print(_("-> Model created"))
    ...
    def onPlayButtonToggled(self, checked):
        if checked:
            self.timer.start()
            self.playButton.text = _("Stop")
        else:
            self.timer.stop()
            self.playButton.text = _("Play")
