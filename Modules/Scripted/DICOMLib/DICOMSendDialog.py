...
class DICOMSendDialog(qt.QDialog):
    ...
    def open(self):
        ...
        self.serverAETitleEdit.text = self.settings.value(_('DICOM/Send/AETitle'), _('CTK'))
        ...
        self.serverAddressLineEdit.text = self.settings.value(_('DICOM/Send/URL'), '')
    ...
    def onOk(self):
        ...
        with slicer.util.tryWithErrorDisplay(_("DICOM sending failed.")):
            ...
            logging.debug(_("DICOM sending of %s files succeeded") % len(self.files))
            self.close()
